...
router.get('/debug', async (req: Request, res: Response) => {
  ...
  res.json({
    ...
    recent_items: recent.map(qi => ({ id: qi.id, user_id: qi.user_id, status: qi.status, created_ago: Math.round(now - qi.created_at), started_ago: qi.started_at ? Math.round(now - qi.started_at) : null, error: qi.error, llm_output: String((qi.params as any)?._llm_output || ''), image1: (qi.params as any)?.image1_name || null, image2: (qi.params as any)?.image2_name || null })),
